...
const ANDROID_HAPTICS: Record<HapticType, Haptics.AndroidHaptics> = {
...
function perform(intent: HapticType): Promise<void> {
  if (isAndroid) {
 ...
 */
function triggerHaptic(intent: HapticType = 'impact-light'): void {
  try {
